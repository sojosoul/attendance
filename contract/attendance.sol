...
contract Attendance {
    ...
    event CheckIn(address indexed wallet, uint256 tanggal, uint256 timestamp);
    event CheckOut(address indexed wallet, uint256 tanggal, uint256 timestamp);
    ...
    function checkIn() public onlyNFTHolder {
        ...
        emit CheckIn(msg.sender, tanggal, block.timestamp);
    } 

    function checkOut() public onlyNFTHolder {
        uint256 tanggal = getTanggalHariIni();
        AttendanceRecord storage rec = records[msg.sender][tanggal];
        ...
        emit CheckOut(msg.sender, tanggal, block.timestamp);
    }
